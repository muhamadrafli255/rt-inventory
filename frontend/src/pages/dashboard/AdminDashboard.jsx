...
export default function AdminDashboard() {
  ...
  const fetchDashboard = useCallback(async (isRefresh = false) => {
    try {
      ...
      // Menyesuaikan jika getDashboard mengembalikan response Axios
      ...
      setError(
        err.response?.data?.message ||
          "Gagal mengambil data dashboard admin. Silakan coba lagi."
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const borrowedStock = Math.max(totalStock - availableStock, 0);

  const totalLoanStatistics = useMemo(() => {
    return loanStatistics.reduce(
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">
            Pantau inventaris, warga, dan seluruh aktivitas peminjaman RT.
          </p>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Distribusi status seluruh peminjaman.
              </p>
            </div>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-lg bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-700">
                    {item.totalBorrowed || item.loanCount || 0}x
                  </span>
                </div>
              </div>
            ))}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-slate-100 p-6">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Aktivitas peminjaman terakhir dari seluruh warga.
            </p>
          </div>
